...
class c_TGIS : public Object{
	public:
	c_StringMap* m_fCountries;
	c_TGIS();
	c_TGIS* m_new();
	void p_LoadCountry(String);
	void p_Load();
	c_TPoint* p_Translate(c_TPoint*);
	void p_Render();
	void mark();
};
class c_Deque : public Object{
	...
	c_Deque();
	c_Deque* m_new();
	c_Deque* m_new2(Array<String >);
	int p_Length();
	void p_Grow();
	void p_PushLast(String);
	bool p_IsEmpty();
	static String m_NIL;
	String p_PopFirst();
	void mark();
};
class c_StringDeque : public c_Deque{
	public:
	c_StringDeque();
	c_StringDeque* m_new();
...
Array<String > bb_filesystem__LoadDir(String);
int bb_filesystem_FileType(String);
Array<String > bb_filesystem_LoadDir(String,bool,bool);
...
class c_TCountry : public Object{
	public:
	c_List* m_fPortions;
	c_TCountry();
	c_TCountry* m_new();
...
class c_TPortion : public Object{
	public:
	c_List2* m_fPoints;
	c_TPortion();
	c_TPortion* m_new();
...
class c_List : public Object{
	...
	c_List();
	c_List* m_new();
	c_Node2* p_AddLast(c_TPortion*);
	c_List* m_new2(Array<c_TPortion* >);
...
class c_Node2 : public Object{
	...
	c_TPortion* m__data;
	c_Node2();
	c_Node2* m_new(c_Node2*,c_Node2*,c_TPortion*);
	c_Node2* m_new2();
	void mark();
};
...
extern Float bb_Globals_gInitialRound;
class c_TPoint : public Object{
	public:
	Float m_latitude;
	Float m_longitude;
	c_TPoint();
	c_TPoint* m_new(Float,Float);
	c_TPoint* m_new2();
	void mark();
};
class c_List2 : public Object{
	...
	c_List2();
	c_List2* m_new();
	c_Node3* p_AddLast2(c_TPoint*);
	c_List2* m_new2(Array<c_TPoint* >);
...
class c_Node3 : public Object{
	...
	c_TPoint* m__data;
	c_Node3();
	c_Node3* m_new(c_Node3*,c_Node3*,c_TPoint*);
	c_Node3* m_new2();
	void mark();
};
...
class c_Node4 : public Object{
	...
	c_Node4* m_parent;
	c_Node4();
	c_Node4* m_new(String,c_TCountry*,int,c_Node4*);
	c_Node4* m_new2();
	c_Node4* p_NextNode();
	void mark();
};
class c_TInput : public Object{
	public:
	bool m_fDragging;
	Float m_fDragStartX;
	Float m_fDragStartY;
...
extern Float bb_Globals_gScale;
extern Float bb_Globals_gScrollVal;
extern Float bb_Globals_gCentreX;
extern Float bb_Globals_gCentreY;
...
class c_Enumerator2 : public Object{
	...
	bool p_HasNext();
	c_TPoint* p_NextObject();
	void mark();
};
extern Float bb_Globals_gRound;
...
int c_TGame::p_OnCreate(){
	...
	gc_assign(m_fGIS,(new c_TGIS)->m_new());
	m_fGIS->p_Load();
	gc_assign(m_fInput,(new c_TInput)->m_new());
	return 0;
}
...
c_TGIS::c_TGIS(){
	m_fCountries=(new c_StringMap)->m_new();
}
c_TGIS* c_TGIS::m_new(){
	return this;
}
void c_TGIS::p_LoadCountry(String t_filename){
	c_FileStream* t_f=c_FileStream::m_Open(String(L"data/GIS/",9)+t_filename+String(L".txt",4),String(L"r",1));
	if(t_f==0){
		bbPrint(String(L"Error opening GIS file: ",24)+t_filename);
	}else{
		c_TCountry* t_country=(new c_TCountry)->m_new();
		c_TPortion* t_portion=(new c_TPortion)->m_new();
		Float t_px=FLOAT(.0);
		Float t_py=FLOAT(.0);
		bool t_previous=false;
		while(!((t_f->p_Eof())!=0)){
			String t_s=t_f->p_ReadLine();
			if(t_s==String(L"---",3)){
				t_country->m_fPortions->p_AddLast(t_portion);
				t_portion=(new c_TPortion)->m_new();
				t_previous=false;
			}else{
				Array<String > t_ps=t_s.Split(String(L",",1));
				Float t_x=Float(int((t_ps[1]).ToFloat()*bb_Globals_gInitialRound))/bb_Globals_gInitialRound;
				Float t_y=Float(int((t_ps[0]).ToFloat()*bb_Globals_gInitialRound))/bb_Globals_gInitialRound;
				c_TPoint* t_p=(new c_TPoint)->m_new(t_x,t_y);
				t_portion->m_fPoints->p_AddLast2(t_p);
				t_previous=true;
				t_px=t_x;
				t_py=t_y;
			}
		...
		t_country->m_fPortions->p_AddLast(t_portion);
		m_fCountries->p_Add(t_filename,t_country);
		bbPrint(String(L"Loaded GIS file: ",17)+t_filename);
		t_f->p_Close();
	}
}
void c_TGIS::p_Load(){
	Array<String > t_=bb_filesystem_LoadDir(String(L"data/GIS",8),false,false);
	int t_2=0;
	while(t_2<t_.Length()){
		String t_s=t_[t_2];
		t_2=t_2+1;
		Array<String > t_sa=t_s.Split(String(L".txt",4));
		p_LoadCountry(t_sa[0]);
	}
}
c_TPoint* c_TGIS::p_Translate(c_TPoint* t_p){
	Float t_x=Float(int(t_p->m_longitude*bb_Globals_gRound))/bb_Globals_gRound*bb_Globals_gScale+bb_Globals_gCentreX+Float(bb_AppGlobals_gHalfScreenWidth);
	Float t_y=-(Float(int(t_p->m_latitude*bb_Globals_gRound))/bb_Globals_gRound)*bb_Globals_gScale+bb_Globals_gCentreY+Float(bb_AppGlobals_gHalfScreenHeight);
	c_TPoint* t_po=(new c_TPoint)->m_new(t_x,t_y);
	return t_po;
}
void c_TGIS::p_Render(){
	Float t_x=FLOAT(.0);
	Float t_y=FLOAT(.0);
	int t_pr1=0;
	int t_pr2=0;
	c_ValueEnumerator* t_=m_fCountries->p_Values()->p_ObjectEnumerator();
	while(t_->p_HasNext()){
		c_TCountry* t_c=t_->p_NextObject();
		c_Enumerator* t_2=t_c->m_fPortions->p_ObjectEnumerator();
		while(t_2->p_HasNext()){
			c_TPortion* t_p=t_2->p_NextObject();
			int t_i=0;
			c_Enumerator2* t_3=t_p->m_fPoints->p_ObjectEnumerator();
			while(t_3->p_HasNext()){
				c_TPoint* t_po=t_3->p_NextObject();
				c_TPoint* t_pt=p_Translate(t_po);
				if(t_i==0){
					t_x=t_pt->m_longitude;
					t_y=t_pt->m_latitude;
				}else{
					if(t_pt->m_longitude==t_x && t_pt->m_latitude==t_y){
						t_pr1+=1;
					}else{
						bb_graphics_DrawLine(t_x,t_y,t_pt->m_longitude,t_pt->m_latitude);
						t_x=t_pt->m_longitude;
						t_y=t_pt->m_latitude;
						t_pr2+=1;
					}
	...
	bbPrint(String(L"Rendered ",9)+String(t_pr2)+String(L", ignored ",10)+String(t_pr1));
}
void c_TGIS::mark(){
	Object::mark();
	gc_mark_q(m_fCountries);
}
c_Deque::c_Deque(){
...
c_Deque* c_Deque::m_new2(Array<String > t_arr){
...
void c_Deque::p_PushLast(String t_value){
...
String c_Deque::m_NIL;
String c_Deque::p_PopFirst(){
...
Array<String > bb_filesystem__LoadDir(String t_path){
	return BBFileSystem::LoadDir(bb_filesystem_FixPath(t_path));
}
int bb_filesystem_FileType(String t_path){
	return BBFileSystem::FileType(bb_filesystem_FixPath(t_path));
}
Array<String > bb_filesystem_LoadDir(String t_path,bool t_recursive,bool t_hidden){
	c_StringDeque* t_dirs=(new c_StringDeque)->m_new();
	c_StringStack* t_files=(new c_StringStack)->m_new2();
	if(!t_path.EndsWith(String(L"/",1))){
		t_path=t_path+String(L"/",1);
	}
	t_dirs->p_PushLast(String());
	while(!t_dirs->p_IsEmpty()){
		String t_dir=t_dirs->p_PopFirst();
		Array<String > t_=bb_filesystem__LoadDir(t_path+t_dir);
		int t_2=0;
		while(t_2<t_.Length()){
			String t_f=t_[t_2];
			t_2=t_2+1;
			...
			String t_p=t_dir+t_f;
			t_files->p_Push4(t_p);
			if(t_recursive && bb_filesystem_FileType(t_path+t_p)==2){
				t_dirs->p_PushLast(t_p+String(L"/",1));
			}
		}
	}
...
String c_Stream::p_ReadLine(){
	c_Stack3* t_buf=(new c_Stack3)->m_new();
	while(!((p_Eof())!=0)){
...
c_TCountry::c_TCountry(){
	m_fPortions=(new c_List)->m_new();
}
c_TCountry* c_TCountry::m_new(){
	return this;
}
void c_TCountry::mark(){
	Object::mark();
	gc_mark_q(m_fPortions);
}
c_TPortion::c_TPortion(){
	m_fPoints=(new c_List2)->m_new();
}
c_TPortion* c_TPortion::m_new(){
	return this;
}
void c_TPortion::mark(){
	Object::mark();
	gc_mark_q(m_fPoints);
}
c_Stack3::c_Stack3(){
...
c_Node2* c_List::p_AddLast(c_TPortion* t_data){
	return (new c_Node2)->m_new(m__head,m__head->m__pred,t_data);
}
c_List* c_List::m_new2(Array<c_TPortion* > t_data){
	Array<c_TPortion* > t_=t_data;
	int t_2=0;
	while(t_2<t_.Length()){
		c_TPortion* t_t=t_[t_2];
		t_2=t_2+1;
		p_AddLast(t_t);
	}
	return this;
...
c_Node2* c_Node2::m_new(c_Node2* t_succ,c_Node2* t_pred,c_TPortion* t_data){
...
Float bb_Globals_gInitialRound;
c_TPoint::c_TPoint(){
	m_latitude=FLOAT(.0);
	m_longitude=FLOAT(.0);
}
c_TPoint* c_TPoint::m_new(Float t__longitude,Float t__latitude){
	m_latitude=t__latitude;
	m_longitude=t__longitude;
	return this;
}
c_TPoint* c_TPoint::m_new2(){
	return this;
}
void c_TPoint::mark(){
	Object::mark();
...
c_Node3* c_List2::p_AddLast2(c_TPoint* t_data){
	return (new c_Node3)->m_new(m__head,m__head->m__pred,t_data);
}
c_List2* c_List2::m_new2(Array<c_TPoint* > t_data){
	Array<c_TPoint* > t_=t_data;
	int t_2=0;
	while(t_2<t_.Length()){
		c_TPoint* t_t=t_[t_2];
		t_2=t_2+1;
...
c_Node3* c_Node3::m_new(c_Node3* t_succ,c_Node3* t_pred,c_TPoint* t_data){
	gc_assign(m__succ,t_succ);
	gc_assign(m__pred,t_pred);
	gc_assign(m__succ->m__pred,this);
	gc_assign(m__pred->m__succ,this);
...
bool c_Map2::p_Add(String t_key,c_TCountry* t_value){
...
c_Node4* c_Node4::m_new(String t_key,c_TCountry* t_value,int t_color,c_Node4* t_parent){
	this->m_key=t_key;
	gc_assign(this->m_value,t_value);
	this->m_color=t_color;
	gc_assign(this->m_parent,t_parent);
...
c_TInput::c_TInput(){
	m_fDragging=false;
	m_fDragStartX=FLOAT(.0);
	m_fDragStartY=FLOAT(.0);
}
c_TInput* c_TInput::m_new(){
	return this;
}
void c_TInput::p_Check(){
	if((bb_input_MouseDown(0))!=0){
		if(!m_fDragging){
			m_fDragStartX=bb_input_MouseX();
			m_fDragStartY=bb_input_MouseY();
			m_fDragging=true;
		}else{
			bb_Globals_gCentreX-=(m_fDragStartX-bb_input_MouseX())*bb_Globals_gScale/bb_Globals_gScrollVal;
			bb_Globals_gCentreY-=(m_fDragStartY-bb_input_MouseY())*bb_Globals_gScale/bb_Globals_gScrollVal;
			m_fDragStartX=bb_input_MouseX();
			m_fDragStartY=bb_input_MouseY();
		}
	}else{
...
Float bb_Globals_gScale;
Float bb_Globals_gScrollVal;
Float bb_Globals_gCentreX;
Float bb_Globals_gCentreY;
...
c_TCountry* c_ValueEnumerator::p_NextObject(){
	c_Node4* t_t=m_node;
...
c_TPortion* c_Enumerator::p_NextObject(){
	c_TPortion* t_data=m__curr->m__data;
	gc_assign(m__curr,m__curr->m__succ);
	return t_data;
}
...
c_TPoint* c_Enumerator2::p_NextObject(){
	c_TPoint* t_data=m__curr->m__data;
	gc_assign(m__curr,m__curr->m__succ);
	return t_data;
}
...
int bbInit(){
	...
	c_Deque::m_NIL=String();
	c_Stream::m__tmp=(new c_DataBuffer)->m_new(4096);
	bb_Globals_gInitialRound=FLOAT(10.0);
	bb_Globals_gScale=FLOAT(3.0);
	bb_Globals_gScrollVal=FLOAT(3.0);
	bb_Globals_gCentreX=FLOAT(0.0);
	bb_Globals_gCentreY=FLOAT(0.0);
	bb_Globals_gRound=FLOAT(10.0);
	return 0;
}
void gc_mark(){
