...
static inline void TraceDumpStackWithError( sCallStackWithErrorCode_t *psStack ) {
    ...
    // Dump registers
    KPrint( NULL, "EAX: 0x%X  EBX: 0x%X  ECX: 0x%X  EDX: 0x%X\n", psStack->eax, psStack->ebx, psStack->ecx, psStack->edx );
    KPrint( NULL, "EBP: 0x%X  ESP: 0x%X  ESI: 0x%X  EDI: 0x%X\n\n", psStack->ebp, psStack->esp, psStack->esi, psStack->edi );
    ...
    for ( i = 0;i < 32; i++ ) {
        n = pStack[ i ];
        KPrint( NULL, "0x%08X: 0x%08X", i * 4, n );
        n = pStack[ i++ ];
        KPrint( NULL, " 0x%08X", n );
        n = pStack[ i++ ];
        KPrint( NULL, " 0x%08X", n );
        n = pStack[ i++ ];
        KPrint( NULL, " 0x%08X\n", n );
...
void Exception_Debug( sCallStack_t *psStack ) {
    ...
    KPrint( "EXCEPTION", "Debug Fault from 0x%X:0x%X - Halting\n" , psStack->cs, psStack->eip );
}
...
void Exception_InvalidOpCode( sCallStack_t *psStack ) {
    ...
    KPrint( "EXCEPTION", "Invalid Opcode from 0x%X:0x%X - Halting\n", psStack->cs, psStack->eip );
}
...
void Exception_Double( sCallStack_t *psStack ) {
    ...
    // Page fault: Need to map in page from VM
    KPrint( "EXCEPTION", "Double Fault from 0x%X:0x%X - Halting\n", psStack->cs, psStack->eip );
}
...
void Exception_StackFault( sCallStack_t *psStack ) {
    ...
    // Debug fault: caused by trap bit in TSS
    KPrint( "EXCEPTION", "Stack Fault from 0x%X:0x%X - Halting\n" , psStack->cs, psStack->eip );
}
...
void Exception_Page( sCallStackWithErrorCode_t *psStack ) {
    ...
    uint32 nFlags = g_sKernel.m_pnPageTables[ nPT + nPTOffset ] & 0xE00;
    ...
    case MM_PAGETYPE_ZEROFILL:
        ...
        break;
    default:
        KPrint( "EXCEPTION", "Unhandled Page Fault at 0x%X from 0x%X:0x%X (Error 0x%X, Flags 0x%X)\n", nCR2, psStack->cs, psStack->eip, psStack->error, nFlags );
        halt();
        break;
    }
}
...
void Exception_GPF( sCallStackWithErrorCode_t *psStack ) {
    ...
    KPrint( "EXCEPTION", "GPF from 0x%X:0x%X - Halting\n", psStack->cs, psStack->eip );
    halt();
}
